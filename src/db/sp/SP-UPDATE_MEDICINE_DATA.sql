CREATE DEFINER=`root`@`localhost` PROCEDURE `SP-UPDATE_MEDICINE_DATA`(p_medicine_id varchar(255), p_medicine_name varchar(255), p_available_pharmacies varchar(255), p_medicine_mrp float(15,2), p_medicine_manufacturer varchar(255), p_medicine_composition varchar(255), p_medicine_packing_type varchar(255), p_medicine_packaging varchar(255))
BEGIN 
    ...
    ON DUPLICATE KEY 
        UPDATE medicine_entity.available_pharmacies = 
            CASE WHEN medicine_entity.available_pharmacies IS NOT NULL AND medicine_entity.available_pharmacies <> "" 
